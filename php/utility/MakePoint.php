...
// Covid19Data SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class Covid19DataMakePoint
{
    public static function call(Covid19DataContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof Covid19DataError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = Covid19DataHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = Covid19DataHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));

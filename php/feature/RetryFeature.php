...
class Covid19DataRetryFeature extends Covid19DataBaseFeature
{
    ...
    public function init(Covid19DataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (Covid19DataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(Covid19DataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(Covid19DataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

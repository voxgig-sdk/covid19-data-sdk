...
// Covid19Data SDK Historical entity
...
class HistoricalEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new HistoricalEntity($this->_client, $opts);
    }

    /**
     * @param Historical|array $args Historical data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = Covid19DataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Historical|array The current Historical data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Historical fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = Covid19DataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Historical fields).
     */
    ...
    /**
     * Load a single Historical.
     *
     * @param HistoricalLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed HistoricalLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Historical|array The loaded Historical as an assoc-array at the
     *   SDK boundary; throws Covid19DataError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = Covid19DataHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }

...
func TestHistoricalEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set COVID__DATA_TEST_HISTORICAL_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func historicalBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COVID__DATA_TEST_HISTORICAL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COVID__DATA_TEST_HISTORICAL_ENTID"])

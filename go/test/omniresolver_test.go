...
import (
	...
	omni "github.com/voxgig-sdk/covid19-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.Covid19DataError); is {
				out := map[string]any{
					"name":    "Covid19DataError",
...
func omniCtx(arg any, client *sdk.Covid19DataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.Covid19DataSDK); is {
			client = live

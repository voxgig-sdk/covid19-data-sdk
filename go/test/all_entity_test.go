...
func TestAllEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set COVID19_DATA_TEST_ALL_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		allRef01MatchDt0 := map[string]any{}
		allRef01DataDt0Loaded, err := allRef01Ent.Load(allRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		if allRef01DataDt0Loaded == nil {
...
func allBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COVID19_DATA_TEST_ALL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COVID19_DATA_TEST_ALL_ENTID"])
	...
	if env["COVID19_DATA_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewCovid19DataSDK(core.ToMapAny(mergedOpts))
	}

...
class FtCtrl extends Covid19DataControl
{
...
class FtClient
{
    ...
    private Covid19DataUtility $utility;

    public function __construct(Covid19DataUtility $utility, array $options)
    {
    ...
    public function get_utility(): Covid19DataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (Covid19DataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public Covid19DataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (Covid19DataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = Covid19DataConfig::make_feature($name);
    ...
    private static function buildUrl(Covid19DataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof Covid19DataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new Covid19DataResponse($fetched);
        }
    ...
    private function fail(Covid19DataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (Covid19DataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

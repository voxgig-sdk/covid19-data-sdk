...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = Covid19DataOmni.make_runner(TEST_JSON_FILE, Covid19DataSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    Covid19DataError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : Covid19DataOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = Covid19DataOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,

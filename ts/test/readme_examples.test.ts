...
//     (\`new Covid19DataSDK(...)\` / \`Covid19DataSDK.test(...)\` become
//     \`Covid19DataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"all":{"test01":{"id":"test01"}},"historical":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new Covid19DataSDK(...)\` and any
// \`Covid19DataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+Covid19DataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/Covid19DataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

...
The Lua SDK for the Covid19Data API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:All(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Covid19Data(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── covid19-data_sdk.lua    -- Main SDK module
...
The main module (`covid19-data_sdk`) exports the SDK constructor

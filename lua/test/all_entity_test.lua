...
describe("AllEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set COVID19_DATA_TEST_ALL_ENTID JSON to run live")
      return
    end
    ...
    local all_ref01_match_dt0 = {}
    local all_ref01_data_dt0_loaded, err = all_ref01_ent:load(all_ref01_match_dt0, nil)
    assert.is_nil(err)
    assert.is_not_nil(all_ref01_data_dt0_loaded)
...
function all_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("COVID19_DATA_TEST_ALL_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["COVID19_DATA_TEST_ALL_ENTID"])

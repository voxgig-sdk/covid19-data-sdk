...
local HistoricalEntity = {}
HistoricalEntity.__index = HistoricalEntity


function HistoricalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HistoricalEntity)
  self._name = "historical"
  self._client = client
...
function HistoricalEntity:get_name()
  return self._name
end


function HistoricalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HistoricalEntity.new(self._client, opts)
end
...
function HistoricalEntity:mark_deleted()
  self._deleted = true
end


function HistoricalEntity:deleted()
  return true == self._deleted
end


function HistoricalEntity:data_set(args)
  if args ~= nil then
...
function HistoricalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HistoricalEntity:match_set(args)
  if args ~= nil then
...
function HistoricalEntity:match_get()
...
function HistoricalEntity:stream(action, args, callopts)
...
---@param reqmatch HistoricalLoadMatch
---@param ctrl? table
---@return Historical
---@return string? err
function HistoricalEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function HistoricalEntity:_run_op(ctx, post_done)
  local utility = self._utility

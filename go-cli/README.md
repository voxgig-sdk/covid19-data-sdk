...
boru-driven command-line client **and** interactive REPL for the Covid19Data
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/covid19-data-cli)
make build

# 2. See usage (words, entities, env vars)
./covid19-data-cli --help

# 3. Provide credentials once, via the environment
export COVID19_DATA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./covid19-data-cli load 1 all            # {id:1} shorthand
./covid19-data-cli load '{id:1}' all       # explicit match map

# 5. Override the API base URL for a single call
COVID19_DATA_BASE=https://api.example.com ./covid19-data-cli load 1 all

# 6. No arguments -> interactive REPL
./covid19-data-cli
covid19-data> load 1 all
covid19-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/covid19-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/covid19-data-cli load 1 all
   ```
...
```sh
./covid19-data-cli load 1 all          # scalar shorthand for {id:1}
./covid19-data-cli load '{id:1}' all     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `covid19-data>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `all`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/covid19-data-cli`. |
...
(`./covid19-data-cli <expr>`) and interactively (the REPL), and expressions compose the

// Command Covid19Data-cli is a CLI + REPL for the Covid19Data SDK,
...
const prompt = "covid19-data"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "all historical"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      COVID19_DATA_APIKEY (api key), COVID19_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: COVID19_DATA_APIKEY carries the API key and
	// COVID19_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("COVID19_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("COVID19_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCovid19DataSDK(opts)

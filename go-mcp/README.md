...
[MCP](https://modelcontextprotocol.io) server exposing the Covid19Data SDK as
two agent tools — `covid19-data_list` and `covid19-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/covid19-data-mcp)
make build

# 2. Provide credentials via the environment
export COVID19_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user covid19-data \
  -- /absolute/path/to/covid19-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./covid19-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// covid19-data_list: first page of records
{ "entity": "all" }
{ "entity": "all", "query": { } }

// covid19-data_load: one record by id
{ "entity": "all", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/covid19-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user covid19-data \
     -- "$PWD"/dist/*/covid19-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `covid19-data_list` and `covid19-data_load` tools now appear
   in new sessions. Ask the agent to *"list all using covid19-data"*
   and it calls `covid19-data_list` with `{"entity":"all"}`.
...
```sh
export COVID19_DATA_APIKEY=sk_live_xxx            # API key
export COVID19_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `covid19-data_list` tool
...
### Call the `covid19-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `covid19-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `covid19-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `COVID19_DATA_APIKEY` | API key sent with every request. |
| `COVID19_DATA_BASE` | Optional override of the API base URL. |
...
```sh
./covid19-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"covid19-data_load","arguments":{"entity":"all","query":{"id":1}}}}'
```

...
require_relative 'Covid19Data_types'
...
class Covid19DataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = Covid19DataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = Covid19DataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, Covid19DataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    Covid19DataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = Covid19DataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = Covid19DataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = Covid19DataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => Covid19DataError.new(
        "#{op}_allow",
        "Covid19DataSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = Covid19DataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = Covid19DataError.new(
        "graphql_error", "Covid19DataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = Covid19DataSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end

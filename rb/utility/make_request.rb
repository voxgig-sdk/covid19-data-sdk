...
module Covid19DataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = Covid19DataResponse.new({})
    ...
    if fetch_err
      ...
      response = Covid19DataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = Covid19DataResponse.new(fetched)
    else

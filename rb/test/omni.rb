...
module Covid19DataOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if Covid19DataOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = Covid19DataOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| Covid19DataOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || Covid19DataOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = Covid19DataSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = Covid19DataError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { Covid19DataOmni.errify(err) }

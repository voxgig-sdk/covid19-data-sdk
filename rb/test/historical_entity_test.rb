...
class HistoricalEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set COVID19_DATA_TEST_HISTORICAL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    historical_ref01_data_dt0_load_result = Helpers.to_map(historical_ref01_data_dt0_loaded.respond_to?(:data_get) ? historical_ref01_data_dt0_loaded.data_get : historical_ref01_data_dt0_loaded)
    assert !historical_ref01_data_dt0_load_result.nil?
    assert_equal historical_ref01_data_dt0_load_result["id"], historical_ref01_data["id"]
...
def historical_basic_setup(extra)
  ...
  client = Covid19DataSDK.test(options, extra)
  ...
  entid_env_raw = ENV["COVID19_DATA_TEST_HISTORICAL_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["COVID19_DATA_TEST_HISTORICAL_ENTID"])
  if idmap_resolved.nil?
    idmap_resolved = Helpers.to_map(idmap)
  end

  if env["COVID19_DATA_TEST_LIVE"] == "TRUE"
    merged_opts = Vs.merge([
      {
      },
      extra || {},
    ])
    client = Covid19DataSDK.new(Helpers.to_map(merged_opts))
  end
  ...
  live = env["COVID19_DATA_TEST_LIVE"] == "TRUE"
  {
    ...
    env: env,
    explain: env["COVID19_DATA_TEST_EXPLAIN"] == "TRUE",
    live: live,

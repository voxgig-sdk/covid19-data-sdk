...
class HistoricalEntity
  ...
  def make
    opts = @_entopts.dup
    HistoricalEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = Covid19DataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Historical, Hash] the current Historical data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = Covid19DataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Historical fields)
  def match_get
  ...
  #
  # @param reqmatch [HistoricalLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Historical.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Historical, Hash] the loaded Historical; raises Covid19DataError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = Covid19DataHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

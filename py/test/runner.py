...
class Covid19DataTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            Covid19DataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = Covid19DataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = Covid19DataTestRunner.getenv("COVID19_DATA_TEST_LIVE")
        override = Covid19DataTestRunner.getenv("COVID19_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = Covid19DataTestRunner.getenv(key)
        ...
        explain = Covid19DataTestRunner.getenv("COVID19_DATA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["COVID19_DATA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if Covid19DataTestRunner._test_control is not None:
            return Covid19DataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                Covid19DataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            Covid19DataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = Covid19DataTestRunner.load_test_control()
...
def load_env_local():
    Covid19DataTestRunner.load_env_local()


def env_override(m):
    return Covid19DataTestRunner.env_override(m)


def entity_data(v):
    return Covid19DataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return Covid19DataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return Covid19DataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return Covid19DataTestRunner.load_test_control()


def live_client_options():
    return Covid19DataTestRunner.live_client_options()


def live_delay_ms():
    return Covid19DataTestRunner.live_delay_ms()

    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set COVID19_DATA_TEST_HISTORICAL_ENTID JSON to run live")
        client = setup["client"]
        ...
        historical_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(historical_ref01_data_dt0_loaded))
        assert historical_ref01_data_dt0_load_result is not None
        assert historical_ref01_data_dt0_load_result["id"] == historical_ref01_data["id"]
...
def _historical_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("COVID19_DATA_TEST_HISTORICAL_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("COVID19_DATA_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = Covid19DataSDK(helpers.to_map(merged_opts))

    _live = env.get("COVID19_DATA_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("COVID19_DATA_TEST_EXPLAIN") == "TRUE",
        "live": _live,

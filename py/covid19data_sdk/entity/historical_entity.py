...
from covid19data_sdk.covid19data_types import (
    Historical,
    HistoricalLoadMatch,
)
...
class HistoricalEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return HistoricalEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Historical:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Historical().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
